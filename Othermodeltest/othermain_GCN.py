...
def load_args():
    ...
    if args.outdir != '':
        ...
        if not os.path.exists(outdir):
            os.makedirs(outdir)
        outdir = outdir + '/{}_{}_{}_{}'.format(args.num_layers, args.lr, args.batch_size, args.dim_hidden)
        if not os.path.exists(outdir):
            os.makedirs(outdir)
...
def main():
    ...
    # best_acc = 0
    # best_accs = []
    idx_path = 'GraphiT-dataset-fold-idx/{}/inner_folds/{}-{}-{}.txt'
    test_idx_path = 'GraphiT-dataset-fold-idx/{}/test_idx-{}.txt'
    inner_idx = 1
    ...
    model = GCN(dataset, args.num_layers, args.dim_hidden).to(device)
