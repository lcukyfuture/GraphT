...
class WL():
    ...
    def transform(self, graph):
        ...
        embeddings = []  # To store embeddings for each graph
        hist, L = self.conv0.histogram(x, batch, norm=self.norm)
        embeddings.append(hist)  # Append initial histogram as part of embedding
        ...
        for h, conv in zip(self.hists, self.convs):
            ...
            embeddings.append(hist)  # Append histograms from each WL iteration
            L += l
            H += (h[:, :hist.shape[1]] @ hist[:, :h.shape[1]].T)
        
        H = (H / self.L.sqrt()[:, None]) / L.sqrt()[None, :]
        
        graph_embeddings = torch.cat(embeddings, dim=1)  # Concatenate all histograms to form embeddings
        
        return H, graph_embeddings


# Usage
# Assuming `data` is your dataset containing the graphs
# wl = WL(num_layers=3)
# graph = (data.x, data
