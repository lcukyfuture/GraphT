...
def load_args():
    ...
    parser.add_argument('--kernels', nargs='+', default=['WL', 'SP', 'RW'], help='Kernel types for each head, e.g., "WL SP RW"')
    ...
    if args.outdir != '':
        ...
        outdir = os.path.join(outdir, f'{args.numheads}_{args.lappe}_{kernel_names}_{args.dim_hidden}_{args.wl}_{args.GL_k}_{args.num_layers}l_{args.hop}h_{args.dropout}_{args.lr}_{args.batch_size}')
        if not os.path.exists(outdir):
            os.makedirs(outdir)        
        args.outdir = outdir
...
def val(loader, model, criterion):
    ...
    corr = 0
    with torch.no_grad():
        for data, mask, pe, lap, labels in loader:
            labels = labels.view(-1)
...
def main():
    ...
    train_dataset = GraphDataset(dataset[train_fold_idx], nb_heads=args.numheads)
