...
class WLConv():
    ...
    def fit(self, x: Tensor, edge_index: Adj, batch=None) -> Tensor:
        ...
        # print("X[edge_index[0,:],:].shape:", X[edge_index[0,:],:].shape)
        # print("edge_index.is_contiguous():", edge_index.is_contiguous())

        S = scatter(X[edge_index[0,:],:], edge_index[1,:], dim=0, reduce='sum')# per node histogram 聚合邻居结点的信息
        S = torch.cat([x[:,None]+1,S],dim=-1) #将结点特征和聚合的邻居结点特征拼接
...
#         H =(lD-lS).abs().sum(3).argmin(1)[0,:,0] 
        ...
        Sn = torch.sqrt((S**2).sum(-1,keepdim=True).float()) #L2范数
        S = S/Sn  #L2 norm
        S =  torch.cat([Sn,S],-1) #
        # print("S_shape", S.shape)
        #relabeling
        self.dictn = torch.unique(S,dim=0)
        # print("dict_shape", self.dictn.shape)
        #相似度计算来取代直接使用硬编码
...
#         H[v<1-1e-6] = inv+D.shape[0]
    ...
    def histogram(self, x: Tensor, batch: Optional[Tensor] = None,
        ...
        #将结点颜色像x的batch一样累加起来，区分每个图的每个结点的着色，一个batch中有num_colors种颜色
